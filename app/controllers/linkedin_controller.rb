...
class LinkedinController < ApplicationController
  ...
  def index
    unless LinkedinOauthSetting.find_by(user_id: current_user.id).nil?
  ...
  def generate_linkedin_oauth_url
    if LinkedinOauthSetting.find_by(user_id: current_user.id).nil?
      client = LinkedIn::Client.new('iv6uehul4g5m', 'wtMfG2MbFerSULTC', @@config)
  ...
  def get_client
    linkedin_oauth_setting = LinkedinOauthSetting.find_by(user_id: current_user.id)
    client = LinkedIn::Client.new('iv6uehul4g5m', 'wtMfG2MbFerSULTC', @@config)
  ...
  def get_basic_profile
    bprofile = BasicProfile.find_by(user_id: current_user.id)
    if bprofile.nil?
      client = get_client
  ...
  def get_full_profile
    fprofile = FullProfile.find_by(user_id: current_user.id)
    if fprofile.nil?
      client = get_client
      full_profile = client.profile(fields: %i[associations honors interests])
